...
async function checkBookingConflict(testBooking) {
  ...
  for await (const booking of allBookings) {
    ...
    if (
      (testStartDate <= endDate && testStartDate >= startDate) ||
      (testStartDate < startDate && testEndDate > endDate)
    ) {
      errorList["startDate"] = "Start date conflicts with an existing booking";
    }
    if (
      (testEndDate >= startDate && testEndDate <= endDate) ||
      (testEndDate > endDate && testStartDate < startDate)
    ) {
