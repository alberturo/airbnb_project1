...
export const EditSpotForm = () => {
  ...
  const [previewImg, setPreviewImg] = useState(
    images.find((image) => image.preview).url
  );
  const [img1, setImg1] = useState(
    images.filter((image) => !image.preview)[0]?.url || ""
  );
  const [img2, setImg2] = useState(
    images.filter((image) => !image.preview)[1]?.url || ""
  );
  const [img3, setImg3] = useState(
    images.filter((image) => !image.preview)[2]?.url || ""
  );
  const [img4, setImg4] = useState(
    images.filter((image) => !image.preview)[3]?.url || ""
  );
  ...
  const submitHandler = async (e) => {
    ...
    const otherImages = [img1, img2, img3, img4];
    ...
    const spotResponse = await dispatch(editASpot(editSpot))
      .then((data) => data)
      .catch((err) =>
        err && typeof err.json == "function" ? err.json() : err
      );

    if (spotResponse.errors) {
      setErrors(
        (currentErr) => (currentErr = { ...currentErr, ...spotResponse.errors })
      );
    }
    if (!checkPreview) {
  ...
  return (
    <form className="new-spot-form" onSubmit={submitHandler}>
      <section>
        ...
        <label>
          <div className="label-error">
            <p>Country</p>
            <p style={{ color: "red" }}>{errors.country && errors.country}</p>
          </div>
          <input
            value={country}
            placeholder={"Country"}
        ...
        <div className="lat-long">
          <label id="lat">
            <div className="label-error">
              <p>Latitude</p>
              <p style={{ color: "red" }}>{errors.lat && errors.lat}</p>
            </div>
            <input
              value={lat}
          ...
          <label id="long">
            <div className="label-error">
              <p>Longitude</p>
              <p style={{ color: "red" }}>{errors.lng && errors.lng}</p>
            </div>
            <input
              value={lng}
